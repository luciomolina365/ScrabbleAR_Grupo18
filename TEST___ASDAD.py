        ...
        for i in range(cant):
            ...
            for j in range(cant):
                if (TableroD[(i,j)]["letra"] != None):
                    ...
                    predef = "TEST\\imagenes\\"

                    if TableroD[(i,j)]["trampa"]==True:
                        
                        direccion = predef + TableroD[(i,j)]["tipo_de_recompensa"] + ".png"
                        lista1.append(sg.Button("" , size=(2, 1) , key=(i,j) , pad=(2,3) , button_color = ('black','Dark grey') , image_filename = direccion , image_size=(25, 22)))
